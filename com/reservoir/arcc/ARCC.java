...
/** 
 * Java toolbox to implement clients to ARCC in Java. The
...
public class ARCC {
    ...
    private static int getMode() {
    	...
    	if (mode == null || mode.isEmpty()) {
    		...
    		"Missing/empty meta-data input file defined in $" + 
    			ARCC_METADATA_ENV;
    		return PRODUCE;
    ...
    private static int optionUseMode() {
    	...
    	assert (opts != null && !opts.isEmpty()) : "$" + 
    		ARCC_OPTION_USE_MODE_ENV + 
    	" ill-defined: " + opts + 
    	...
    	if (opts.compareToIgnoreCase("all") == 0) {
    		...
    		throw new Error("$" + ARCC_OPTION_USE_MODE_ENV + " mis-defined: " + 
    			opts +
    			".\n Must be \"all\", \"default\", or \"user-specified\".");
    	}
    }
     ...
     */
    public static void 
    	saveAutotunableOptions(String opt_str) throws IOException {
	...
	// Make sure the path is absolute
    	assert opt_fname.indexOf(File.separator) != -1 : 
    	"Absolute path required for $" + 
    ...
    public static String timingOutputFilename() {
    	...
    	"Absolute path required for $" + 
    		ARCC_PERF_FILE_ENV + " (" + perf_fname + ")," + 
    	File.separator+" not found";
    	return perf_fname;
    }

    // _____________________ Meta-data syntax elements _________________________
    //
    //
    ...
    public static final String 
    	varDef(String vname, Collection<? extends Object> values) {
    ...
    // ____________________________ Various utils ______________________________
    //
    //
